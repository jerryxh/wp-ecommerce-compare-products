...
function wpec_compare_install(){
	update_option('a3rev_wpeccp_version', '2.2.0');
	...
	// Build sass
	global $wpec_compare_less;
	$wpec_compare_less->plugin_build_sass();
...
add_filter( 'plugin_row_meta', array('WPEC_Compare_Hook_Filter', 'plugin_extra_links'), 10, 2 );
	...
	add_action('init', array( 'WPEC_Compare_Hook_Filter', 'nocache_ours_page' ), 0 );
...
function wpec_cp_lite_upgrade_plugin () {
	...
	// Upgrade to 2.2.0
	if(version_compare(get_option('a3rev_wpeccp_lite_version'), '2.2.0') === -1){
		// Build sass
		global $wpec_compare_less;
		$wpec_compare_less->plugin_build_sass();
		update_option('a3rev_wpeccp_lite_version', '2.2.0');
	}
